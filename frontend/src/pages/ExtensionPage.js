...
export default function ExtensionPage() {
    ...
    const fetchKey = useCallback(async () => {
        try {
            const { data } = await api.get('/extension/key');
    ...
    const fetchRecent = useCallback(async () => {
        try {
            const { data } = await api.get('/extension/recent-pushes');
    ...
    const handleCopy = async () => {
        await navigator.clipboard.writeText(extKey).catch(() => {});
        setCopied(true);
    ...
    const handleRegenerate = async () => {
        ...
        try {
            const { data } = await api.post('/extension/regenerate-key', {});
            setExtKey(data.ext_key);
            setShowKey(true);
        } catch (err) { console.error(err); }
